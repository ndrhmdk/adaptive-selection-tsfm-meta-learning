...
class Moirai2Forecaster(BaseForecaster):
    ...
    def __init__(self, checkpoint: str="Salesforce/moirai-2.0-R-small", device: str | None=None):
        if device is None:
    ...
    def predict(self, history: np.ndarray, prediction_length: int) -> ForecastResult:
        ...
        past_target = torch.tensor(clean_history.T, dtype=torch.float32, device=self.device).unsqueeze(-1)
        past_observed_target = torch.tensor(observed_mask.T, dtype=torch.bool, device=self.device).unsqueeze(-1)
        past_is_pad = torch.zeros((n_variates, context_length), dtype=torch.bool, device=self.device)
        ...
        if quantile_predictions.shape != expected_shape:
            raise ValueError(f"Unexpected Moirai forecast shape:\n- Expected: {expected_shape}\n- Received: {quantile_predictions.shape}")
