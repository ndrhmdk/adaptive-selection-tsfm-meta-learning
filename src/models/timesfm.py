...
class TimesFM3Forecaster(BaseForecaster):
    def __init__(self, checkpoint: str="google/timesfm-3.0-pytorch", device: str | None=None, batch_size: int=16):
        if device is None:
        ...
        config = ModelConfig(checkpoint_path=checkpoint, per_core_batch_size=batch_size, device=device)
    ...
    def predict(self, history: np.ndarray, prediction_length: int) -> ForecastResult:
        ...
        '''
        context = history.T
        outputs = list(self.evaluator.predict_batch(
            contexts=[context],
