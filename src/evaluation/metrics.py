...
def mase_per_series(y_true: np.ndarray, y_pred: np.ndarray, history: np.ndarray, seasonal_period: int) -> np.ndarray:
    """
...
def mase(y_true: np.ndarray, y_pred: np.ndarray, history: np.ndarray, seasonal_period: int) -> float:
    scores = mase_per_series(y_true=y_true, y_pred=y_pred, history=history, seasonal_period=seasonal_period)
    return float(np.nanmean(scores))

def evaluate_forecast(y_true: np.ndarray, y_pred: np.ndarray, history: np.ndarray, seasonal_period: int) -> dict[str, float]:
    return {
        "mae": float(mean_absolute_error(y_true, y_pred)),
        "rmse": float(root_mean_squared_error(y_true, y_pred)),
        "smape": smape(y_true, y_pred),
        "mase": mase(y_true=y_true, y_pred=y_pred, history=history, seasonal_period=seasonal_period)}
